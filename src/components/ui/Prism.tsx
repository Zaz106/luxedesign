...
const Prism: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      // Small optimization: check if we are actually hovering before calculating everything? 
      // Actually we need to track mouse relative to center regardless of element bounds for this effect often.
      const ww = window.innerWidth;
      const wh = window.innerHeight;
      const nx = (2.0 * e.clientX) / ww - 1.0;
      const ny = (2.0 * e.clientY) / wh - 1.0; 
      pointer.x = nx;
      pointer.y = ny;
      pointer.inside = true;
    };
